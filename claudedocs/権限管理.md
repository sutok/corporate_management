...
erDiagram
    ...
    roles {
        int id PK
        string code UK "例: user.create"
        string name "例: ユーザー作成"
        text description
        string resource_type "例: user"
        datetime created_at
        datetime updated_at
    }
    ...
    group_roles {
        int id PK
        string role_id
        string group_role_name "例: 管理者グループ"
        text description
        int company_id FK "NULL=システムグループ"
