...
erDiagram
    ...
    User {
        ...
        string name
        string email
        string role "旧カラム（非推奨）"
    }

    Role {
        int id PK
        int company_id FK "NULL=システムロール"
        string code "ロールコード"
        string name "ロール名"
        text description
        bool is_system "システムロールフラグ"
    }
...
- **主な権限**:
  ...
  - 企業情報管理 (view, update)
  - 施設管理 (view, create, update, delete)
  - 施設所属管理 (manage)
...
| コード | 名前 | 説明 |
|--------|------|------|
| `facility.view` | 施設閲覧 | 施設情報を閲覧する |
| `facility.create` | 施設作成 | 新しい施設を作成する |
| `facility.update` | 施設更新 | 施設情報を更新する |
| `facility.delete` | 施設削除 | 施設を削除する |
| `facility_assignment.manage` | 施設所属管理 | ユーザーの施設所属を管理する |
...
**処理内容**:
1. 26個の権限（Permission）を作成
2. 6個のシステムロール（Role）を作成
...
7. ❌ `facilities.py` - 施設管理
8. ❌ `facility_assignments.py` - 施設所属管理
9. ❌ `users.py` - ユーザー管理
10. ❌ その他ルーター
...
user_roles = [
    ...
    UserRole(user_id=user.id, role_id=facility_manager_role.id),
]
